...
```{r}
#Confidence Interval  
ci_data <- data.frame(schooling = 12)
predict(lm_scl, newdata = ci_data, interval = 'confidence')
```
...
Linearity: Scatter plot - satisfied

```{r}

lm_ach <- lm(data = train_data, life_exp ~ (alcohol))
create_plots <- function(model, data) {
# Linearity Plot
  summary_model <- summary(model)
  plot_linearity <- ggplot(data, aes(x = (alcohol), y = life_exp)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE)+
    ggtitle("Life Expectancy with respect to alcohol")+
    labs(x = "Alcohol", y = "Life Expectancy")
  plot_linearity_1<- ggExtra::ggMarginal(plot_linearity, type = "histogram")
...
# Residuals vs. Fitted Plot (Homoscedasticity)
  ...
  return(list(summary = summary_model,linearity = plot_linearity_1, qq = plot_qq, resid_vs_fitted = plot_resid_vs_fitted))
}
...
```{r}
#Confidence Interval  
ci_data_2 <- data.frame(alcohol = 4.6)
predict(lm_ach, newdata = ci_data_2, interval = 'confidence')
```
...
T-test statistic: From above the test statistic value for life expectancy to schooling is 46.83 , and
...
Conclusion: The final estimated model equation is $\hat{life expectancy} = \hat{44.03 }+ \hat{2.09}*Schooling$

**Interpretation:**

Intercept (44.03) : the estimated value of cubed life expectancy value when Schooling is 0.

Slope(2.09) : The estimated change in value of life expectancy for one unit change in value of schooling. in general, for every year increase in schooling the  life expectancy value is expected to increase by 2.09 years.
...
Normality: From the qqplot we can see the plot is curved, with distorted tails, but assuming it normal and continued with model.

T-test statistic: From above the test statistic value for life expectancy to schooling is 19.51, and
...
Conclusion: The final estimated model equation is $\hat{life expectancy} = \hat{63.08 }+ \hat{3.25}*\sqrt{Alcohol}$

Interpretation:

intercept ($\hat{64.72}$): This coefficient is the estimated value of life expectancy when the value of alcohol consumption is zero. It represents the starting point of the relationship between the life expectancy and that of alcohol consumption according to the model's fiting to the data.

Slope($\hat{3.25}$): This coefficient indicates the amount of change in the value of life expectancy for each one-unit increase in the value of alcohol consumption. It suggests that when alcohol consumption increases by one unit (which corresponds to alcohol consumption per liters in years, the model predicts an increase of 0.95 years increase in the value of life expectancy.

But in true case its false.
